...
CanMessage::CanMessage()
  : _is_tx(false), _raw_id(0), _dlc(0), _interface(0), _u64(0)
{
    _timestamp.tv_sec = 0;
...
CanMessage::CanMessage(uint32_t can_id)
  : _is_tx(false), _dlc(0), _interface(0), _u64(0)
{
    _timestamp.tv_sec = 0;
    _timestamp.tv_usec = 0;
    setId(can_id);
...
QString CanMessage::getDirctionString() const
{
    if (isTX()) {
        return QString().sprintf("tx");
    } else {
        return QString().sprintf("rx");
    }
}
